...
import { ForgotPasswordFormData } from "@/app/core/interfaces/auth-interfaces";
import { forgotPasswordSchema } from "@/app/core/schemas/auth-schemas";
...
const ForgotPasswordForm = () => {
  ...
  const onSubmit: SubmitHandler<ForgotPasswordFormData> = async (
    ...
    try {
      ...
      const response = await authService.forgotPassword({usernameOrEmail: data.email, captchaToken: resetToken});
      if (response.success) {
      ...
      <div className="card-content">
        ...
        >
          ...
          <div className="flex flex-col items-center gap-[1rem]">
            <ButtonComponent 
              className="primary w-full" type="submit" label="Enviar" />
          </div>
        </form>
      </div>
    </>
